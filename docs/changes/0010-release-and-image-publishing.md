...
- `CI / ci` (from 0009) is intentionally SKIPPED on the release PR cut by `release-please` (a job-level `if` skips the job on `release-please--*` head branches), because that PR adds only the version bump + CHANGELOG. The trustworthiness gate is therefore that `CI / ci` was green on `main` at the release commit the PR builds on. (Policy amended by the PR that introduced the skip; the prior rule required `CI / ci` to be green on the release PR itself.)
