...
- [x] **PR 2 — Observability: sync-log resolution, tail, and status surface** (no killing) — PR #23
  ...
  - [x] Tests under `test/config/` for every accept and reject case, including the poll-exceeds-timeout pair and the timeout-`0`-so-poll-unconstrained case
  - [x] `src/obsidian/watchdog.ts`: `WatchdogFs` surface, vault-id resolution by `config.json` `vaultPath` match (normalized absolute compare, no prefix match, newest-mtime selection with a lexicographic tiebreak and a `warn`), fail-soft on every filesystem error per the spec's dispositions, incremental tail with start-at-current-size, inode/size rotation reset, per-poll byte cap with a skip `warn`, partial-line buffering, and the `snapshot()` / `stop()` handle
  - [x] `src/obsidian/child.ts`: start the watchdog after a successful spawn, `wd.stop()` in a `finally` around `await handle.exited`, and again from `requestStop()`; extend `snapshot()` with `lastSyncActivityAt` and `watchdog`
  - [x] `src/obsidian/index.ts`: hoist the XDG base resolution out of the `skipAuthBootstrap` branch and pass `syncDir` plus `cfg.syncWatchdog` into every `VaultChild`
  - [x] Propagate the two new `VaultStatus` fields through `src/vault/status.ts` and both adapters; update `test/parity/` fixtures so REST and MCP still agree
  - [x] Tests in `test/obsidian/watchdog.test.ts`: resolution success, ambiguous-match tiebreak, missing directory, unreadable and malformed `config.json`, no-backlog-on-start, mid-poll truncation, inode replacement, over-cap append, partial-line buffering across polls, `lastSyncActivityAt` tracking, and the "unresolved past threshold" `warn`
