...
- `/readyz` gains an `ok` field. No status code changes, no new routes, and no change to `/healthz`.
...
- **No parsing of `sync.log` contents.** Lines are forwarded verbatim. Deriving structured sync state from log text would couple us to an undocumented, unversioned format for information the mtime already provides.
...
- [ ] **PR 2 — Observability: sync-log resolution, tail, and status surface** (no killing)
  ...
  - [ ] `src/http/index.ts`: add `ok` to the `/readyz` body on both paths; tests asserting the exact shape on 200 and 503
