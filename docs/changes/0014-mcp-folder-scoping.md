...
The intended use case is a **memory backend for LLM agents**: each agent gets an agent-scoped folder (`agents/<id>/`) inside a shared vault, writes its notes there, and searches its own memory without seeing — or corrupting — anyone else's.
...
The obvious server-side fixes — an agent registry, per-agent credentials, service accounts — are explicitly rejected by the operator: they add a control plane, a database, and a lifecycle to a single-process server whose whole premise is that there isn't one. The MCP client configuration is trusted, and it is already per-agent (each agent has its own `mcpServers` entry). That makes the client's **connection URL** the natural place to carry the scope, with no per-agent provisioning: the server gains no registry, no credential store, and nothing an operator has to create before an agent can connect. (It is not literally stateless — `src/mcp/index.ts` already keeps a session map, and this change adds a bounded per-scope registry memo — but both are process-lifetime caches keyed by what the client sent, not per-agent configuration.)
...
`:prefix{.+}` is the same Hono named-regex wildcard form the REST adapter already uses for multi-segment paths — there spelled `:path{.+}` on the file routes (`src/http/routes/files.ts:94`) and the folder routes (`src/http/routes/folders.ts:38`) — so the two adapters stay consistent in form, differing only in the parameter name.
...
- The scope MUST be parsed from the **raw, un-normalized request target**, split on `/`, with `decodeURIComponent` applied to each segment exactly once. Two accessors are specifically NOT acceptable: `c.req.param("prefix")`, because Hono decodes route params and folds `%2F` into the path structure so an encoded separator becomes undetectable afterwards; and `new URL(c.req.url).pathname`, because the WHATWG URL parser resolves dot segments — including percent-encoded ones (`%2e%2e` is a double-dot segment per the URL standard) — so the very inputs this rule must reject would be silently collapsed before validation ever sees them. The implementation MUST pin down which accessor preserves the raw target on this runtime (`c.req.path` is the candidate: Hono derives it by slicing the URL string rather than parsing it) and MUST cover that choice with an adapter-level test asserting that `%2e%2e`, `%2F`, and `%252e%252e` survive intact up to the validator. If no accessor preserves it, the scope MUST be rejected rather than parsed from a normalized value.
...
- The decoded segments MUST be joined with `/`, stripped of any trailing `/`, and normalized by dropping empty and single-dot (`.`) segments — and only then validated. The two rules are consistent because they act at different levels: an *empty* segment is an artifact of URL joining (`/mcp/v//agents/a`) and collapses, while a segment whose DECODED CONTENT begins with or contains a separator (`%2F`, `%5C`) is rejected by the rule above and never reaches normalization. So `//agents/a` is an alias for `agents/a`, `%2Fagents/a` is a `400`, and the normalized prefix handed to `assertSafeRelativePath` can never begin with a separator — which is why its leading-separator check is a backstop here, not the primary gate. Order matters in both directions: validating before decoding would let `%2e%2e%2f` through, and decoding a second time would turn a literal `%252e%252e` into `..` after validation had already passed. Exactly one decode, on the raw segment, is the rule. Normalization is what makes `/mcp/v/agents/a`, `/mcp/v/agents/a/`, and `/mcp/v/agents/./a` one scope rather than three — they resolve to the same root, so they MUST produce the same scope key, or the per-scope memo and the session scope-match check below would treat aliases of one scope as distinct.
...
- The scoped resource handler MUST be built with a slug provider returning ONLY the scoped slug. `buildMcpRoutes` today passes `buildResourceHandler` a separate `slugs()` closure over `supervisor.list()` (`src/mcp/index.ts`), which the deps wrapper does not touch — leaving it as-is would let `resources/list` paginate across every configured vault even though every tool is confined. A two-vault scoped `resources/list` test MUST pin this.
  ...
  return {
    ...
    indexer: {
      ...
      search: async (s, q, opts) => {
        ...
        // flatMap, not map+filter: `strip` returns `string | null`, and filtering
        // afterwards would not narrow the type back to `SearchHit[]`.
...
- **Decision:** Carry the scope in the URL path.
  - **Why:** Every MCP client can configure a URL; not every client can set custom headers. The URL is also the one piece of configuration that is already per-agent, and it adds no per-agent provisioning — no registry, no mapping table, nothing an operator creates before an agent connects.
...
- **No read-through / overlay scopes.** A scope is a closed box, not "write here, read everywhere". A read-only view of the wider vault alongside a scoped write root is a plausible follow-up and deliberately deferred.
...
- [ ] **Scope resolution + scoped deps: `src/mcp/scope.ts`**
  ...
  - [ ] `assertScopeRootSafe(scopeRoot, vaultRoot)` wrapping `assertNotSymlinkEscape` (which already tolerates a not-yet-created scope root by walking up the chain — a missing root is an empty scope, not a rejection), plus the per-call wrapper that runs it before every `ToolDefinition.call`, `resources/list`, and `resources/read` in a scoped session
