...
- The scope root MUST be checked for symlink escape when the session is bound AND before every tool call and resource read — the per-operation symlink guards stop at the root they are given, which for a scoped session is the scope root itself, so a bind-time-only check leaves the scope root swappable for the life of the session.
