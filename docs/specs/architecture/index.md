...
site/             # standalone static landing site for ob.fx.gd — own package + lockfile, never in the image (see Landing Site spec)
Dockerfile
biome.json
tsconfig.json
package.json
...
- Biome MUST be the only linter and formatter. `biome.json` MUST extend `recommended`, enable `noExplicitAny`, and ignore: `node_modules/`, `dist/`, `data/`, `coverage/`, `*.md`, `bun.lockb`, lockfiles, `site/`, and any path under `vault/`.
...
| 2026-09-15 | Project Layout gains a `site/` top-level directory for the static landing site at `ob.fx.gd` (planned). It is a standalone package outside the root lint/typecheck/coverage scope, with `site/` excluded from root Biome through `biome.json`'s ignore list, and is never copied into the image; its behavior is owned by the [Landing Site spec](../landing-site/index.md). | [Change 0016](../../changes/0016-landing-site.md) |
